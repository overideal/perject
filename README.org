...
The built-in *savehist* package is used to save and restore variables that get otherwise restored by =desktop.el=.
If =perject-load-at-startup= is set to /'previous/, then you need to add the following lines of code to your Emacs configuration file:
#+BEGIN_SRC emacs-lisp
...
You might want to save and restore some global variables by adding the following lines to the previous =:config= block:
#+BEGIN_SRC emacs-lisp
...
There exist no packages that offer the same functionality of perject, but some provide a small subset of it:
- perject.el (built-in): Despite the name, this refers to *software projects* (e.g. a git repository) and not arbitrary projects in the sense I defined above.
  As such, the package has nothing to do with /perject/.
- [[https://github.com/florommel/bufferlo][bufferlo]]: This defines a buffer list per frame, which can be restored with /desktop.el/.
  Therefore, this package is probably closest to /perject/ in terms of functionality, but instead of the notion of projects and collections
  you just have frames. Functionality like =perject-auto-add-hooks=, tab support etc. seems to be missing from this package.
- [[https://github.com/mclear-tools/tabspaces][tabspaces]]: This package is similar to perject in that it provides "buffer-isolated workspaces".
  ...
  The main difference is again what is considered a project in that this package solely focuses on software projects and not on the generalized version provided by perject.
